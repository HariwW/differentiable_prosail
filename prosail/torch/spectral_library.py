...
def get_spectra():
    ...
    _, nr, kab, kcar, kant, kbrown, kw, km = torch.from_numpy(
        np.loadtxt(BytesIO(prospect_d_spectraf), unpack=True)
    ).float()
    ...
    _, nr, kab, kcar, kant, kbrown, kw, km, kprot, kcbc = torch.from_numpy(
        np.loadtxt(BytesIO(prospect_pro_spectraf), unpack=True)
    ).float()
    ...
    nr, kab, kcar, kbrown, kw, km = torch.from_numpy(
        np.loadtxt(BytesIO(prospect_5_spectraf), unpack=True)
    ).float()
    ...
    rsoil1, rsoil2 = torch.from_numpy(
        np.loadtxt(BytesIO(soil_spectraf), unpack=True)
    ).float()
