...
def volscatt(
    ...
    bto[tto_inf_90] = torch.full_like(bto[tto_inf_90], np.pi)
    ...
    bt2_sup_0 = bt2 > 0.0
    t2[bt2_sup_0] = torch.sin(bt2[bt2_sup_0]) * (
        2.0 * ds[bt2_sup_0] * do_[bt2_sup_0]
        + ss[bt2_sup_0]
        * so[bt2_sup_0]
        * torch.cos(bt1[bt2_sup_0])
        * torch.cos(bt3[bt2_sup_0])
    )
...
def weighted_sum_over_lidf(
    ...
    litab = (
        (
            torch.arange(n_angles, dtype=tts.dtype, device=tts.device) * angle_step
            + (angle_step * 0.5)
        ).repeat(lidf.size(0), 1)
    )
...
def hotspot_calculations(
    alf: torch.Tensor, lai: torch.Tensor, ko: torch.Tensor, ks: torch.Tensor
    ...
    n_steps = 20
    x2 = alf.new_ones(alf.size(0), n_steps)
    fint = ((1.0 - torch.exp(-alf)) * 0.05).repeat(1, n_steps)
    sumint = alf.new_zeros(alf.size(0), n_steps)
    x2[:, :-1] = (
        -torch.log(
            1.0
            - torch.arange(1, n_steps, dtype=alf.dtype, device=alf.device).unsqueeze(0)
            * fint[:, :-1]
        )
        / alf
    )
    y2 = (-(ko + ks) * lai).reshape(-1, 1) * x2 + fhot.reshape(-1, 1) * (
        1.0 - torch.exp(-alf * x2)
    ) / alf
    f2 = torch.exp(y2)

    x1 = alf.new_zeros(alf.size(0), n_steps)
    x1[:, 1:] = x2[:, :-1]
    y1 = alf.new_zeros(alf.size(0), n_steps)
    y1[:, 1:] = y2[:, :-1]
    f1 = alf.new_ones(alf.size(0), n_steps)
    f1[:, 1:] = f2[:, :-1]
    EPS = alf.new_tensor(1e-8)
...
def Jfunc1(k, L, t):
    ...
    if nb > 1:
        ...
        result[del_sup_1em3] = (
            torch.exp((-L * t_ext)[del_sup_1em3])
            - torch.exp(-k_ext * t_ext)[del_sup_1em3]
        ) / ((k_ext - L)[del_sup_1em3])
    return result
...
def verhoef_bimodal(
    a: torch.Tensor, b: torch.Tensor, n_elements: int = 18
) -> torch.Tensor:
...
def campbell(alpha: torch.Tensor, n_elements: int = 18) -> torch.Tensor:
    ...
    """
    excent = torch.exp(
        -1.6184e-5 * alpha**3.0 + 2.1145e-3 * alpha**2.0 - 1.2390e-1 * alpha + 3.2491
    ).reshape(-1, 1)
    freq = alpha.new_zeros(alpha.size(0), n_elements)
    step = 90.0 / n_elements
    degrees = (
        torch.arange(
            0, n_elements + 1, dtype=alpha.dtype, device=alpha.device
        ).repeat(alpha.size(0), 1)
        * step
    )
    ...
    EPS = alpha.new_tensor(
        1e-8
    )  # Quickfix for alph divergence with excent = 1 (alpha approx 58.435)
    ...
    if len(excent_eql_1) > 0:
        freq[excent_eql_1, :] = torch.abs(
            torch.cos(tl1[excent_eql_1]) - torch.cos(tl2[excent_eql_1])
        )

    excent_sup_1 = torch.where(excent > 1)[0]
    if len(excent_sup_1) > 0:
        dum[excent_sup_1, :] = dum[excent_sup_1, :] + alph2[excent_sup_1] * torch.log(
            x1[excent_sup_1, :] + alsx1[excent_sup_1, :]
        )
        freq[excent_sup_1, :] = torch.abs(
            dum[excent_sup_1, :]
            - (
                x2[excent_sup_1, :] * alsx2[excent_sup_1, :]
                + alph2[excent_sup_1]
                * torch.log(x2[excent_sup_1, :] + alsx2[excent_sup_1, :])
            )
        )

    excent_inf_1 = torch.where(excent < 1)[0]
    if len(excent_inf_1) > 0:
        dum[excent_inf_1, :] = dum[excent_inf_1, :] + alph2[
            excent_inf_1
        ] * torch.arcsin(x1[excent_inf_1, :] / alph[excent_inf_1])
        freq[excent_inf_1, :] = torch.abs(
            dum[excent_inf_1, :]
            - (
                x2[excent_inf_1, :] * alsx2[excent_inf_1, :]
                + alph2[excent_inf_1]
                * torch.arcsin(x2[excent_inf_1, :] / alph[excent_inf_1])
            )
        )
...
def foursail(rho, tau, lidfa, lidfb, lidftype, lai, hotspot, tts, tto, psi, rsoil):
    """
    Parameters
    ----------
    rho : array_like
    ...
    if lidftype == 1:
        raise NotImplementedError(
            "Vectorizing this equation is impossible for the moment"
        )
    ...
    sigf = torch.clamp_min(sigf, torch.finfo(sigf.dtype).tiny)
    sigb = torch.clamp_min(sigb, torch.finfo(sigb.dtype).tiny)
    ...
    tss = torch.ones_like(sob)
    too = torch.ones_like(sob)
    tsstoo = torch.ones_like(sob)
    rdd = torch.zeros_like(m)
    tdd = torch.ones_like(m)
    rsd = torch.zeros_like(m)
    tsd = torch.zeros_like(m)
    rdo = torch.zeros_like(m)
    tdo = torch.zeros_like(m)
    rso = torch.zeros_like(m)
    rsos = torch.zeros_like(m)
    rsod = torch.zeros_like(m)
    rddt = rsoil.clone()
    rsdt = rsoil.clone()
    rdot = rsoil.clone()
    rsodt = torch.zeros_like(m)
    rsost = rsoil.clone()
    rsot = rsoil.clone()
    ...
    if len(lai_sup_0) > 0:
        ...
        rsod_pos = (T1 + T2 - T3) / (1.0 - rinf2)
        # Thermal "sod" quantity
        T4 = Tv1 * (1.0 + rinf)
        T5 = Tv2 * (1.0 + rinf)
        T6 = (rdo[lai_sup_0, :] * J2ks + tdo[lai_sup_0, :] * J1ks) * (1.0 + rinf) * rinf
        gammasod_pos = (T4 + T5 - T6) / (1.0 - rinf2)
        # Treatment of the hotspot-effect
        alf = torch.full_like(tss, 1e36)
        # Apply correction 2/(K+k) suggested by F.-M. Breon
        hotspot_sup_0 = torch.where((hotspot > 0.0) * (lai > 0))[0]
        if len(hotspot_sup_0) > 0:
            alf[hotspot_sup_0] = (
                (dso[hotspot_sup_0] / hotspot[hotspot_sup_0])
                * 2.0
                / (ks[hotspot_sup_0] + ko[hotspot_sup_0])
            )

        alf_eql_0 = torch.where((alf == 0.0) * (lai > 0))[0]
        sumint = torch.zeros_like(tss)
        if len(alf_eql_0) > 0:
            tsstoo[alf_eql_0] = tss[alf_eql_0]
            sumint[alf_eql_0] = (1.0 - tss[alf_eql_0]) / (
                ks[alf_eql_0] * lai[alf_eql_0]
            )

        alf_nql_0 = torch.where((alf != 0.0) * (lai > 0))[0]
        if len(alf_nql_0) > 0:
            tsstoo_n0, sumint_n0 = hotspot_calculations(
                alf[alf_nql_0], lai[alf_nql_0], ko[alf_nql_0], ks[alf_nql_0]
            )
        ...
        rsos_pos = w[lai_sup_0, :] * lai[lai_sup_0] * sumint[lai_sup_0]
        gammasos = ko[lai_sup_0] * lai[lai_sup_0] * sumint[lai_sup_0]
        # Total canopy contribution
        rsos[lai_sup_0, :] = rsos_pos
        rsod[lai_sup_0, :] = rsod_pos
        rso[lai_sup_0, :] = rsos_pos + rsod_pos
        gammaso[lai_sup_0, :] = gammasos + gammasod_pos
        ...
        if (dn < 1e-8).any():
            pass
        dn = torch.clamp_min(dn, torch.finfo(dn.dtype).tiny)
        rddt[lai_sup_0, :] = (
            rdd[lai_sup_0, :]
            + tdd[lai_sup_0, :] * rsoil[lai_sup_0, :] * tdd[lai_sup_0, :] / dn
        )
        rsdt[lai_sup_0, :] = (
            rsd[lai_sup_0, :]
            + (tsd[lai_sup_0, :] + tss[lai_sup_0, :])
            * rsoil[lai_sup_0, :]
            * tdd[lai_sup_0, :]
            / dn
        )
        rdot[lai_sup_0, :] = (
            ...
            + tdd[lai_sup_0, :]
            * rsoil[lai_sup_0, :]
            * (tdo[lai_sup_0, :] + too[lai_sup_0, :])
            / dn
        )
        rsodt[lai_sup_0, :] = (
            (
                (tss[lai_sup_0] + tsd[lai_sup_0, :]) * tdo[lai_sup_0, :]
                + (
                    tsd[lai_sup_0, :]
                    + tss[lai_sup_0] * rsoil[lai_sup_0, :] * rdd[lai_sup_0, :]
                )
                * too[lai_sup_0]
            )
            * rsoil[lai_sup_0, :]
        ...
        rsost[lai_sup_0, :] = (
            rso[lai_sup_0, :] + tsstoo[lai_sup_0, :] * rsoil[lai_sup_0, :]
        )
