...
from .prospect_d import init_prospect_spectra, run_prospect, subsample_spectra
...
def init_prosail_spectra(
    ...
    [nr, kab, kcar, kbrown, kw, km, kant, kprot, kcbc, lambdas] = init_prospect_spectra(
        nr=nr,
        kab=kab,
        kcar=kcar,
        kbrown=kbrown,
    ...
    soil_spectrum1 = (
        spectral_lib.soil.rsoil1.to(device)
        if soil_spectrum1 is None
        else soil_spectrum1
    )
    soil_spectrum2 = (
        spectral_lib.soil.rsoil2.to(device)
        if soil_spectrum2 is None
        else soil_spectrum2
    )
    if R_down > 1:
...
def run_prosail(
    ...
    if factor not in ("SDR", "BHR", "DHR", "HDR", "ALL", "ALLALL"):
        raise ValueError("'factor' must be one of SDR, BHR, DHR, HDR, ALL or ALLALL")
    if init_spectra:
        [
            soil_spectrum1,
            soil_spectrum2,
    ...
    lidfb = torch.as_tensor(lidfb, dtype=N.dtype, device=device)
    alpha = torch.as_tensor(alpha, dtype=N.dtype, device=device)
    if rsoil0 is None:
        if (rsoil is None) or (psoil is None):
            raise ValueError(
                "If rsoil0 isn't defined, then rsoil and psoil" " need to be defined!"
            )
        soil_spectrum1 = soil_spectrum1.to(device=device, dtype=N.dtype)
        soil_spectrum2 = soil_spectrum2.to(device=device, dtype=N.dtype)
        rsoil0 = rsoil * (psoil * soil_spectrum1 + (1.0 - psoil) * soil_spectrum2)
    else:
        rsoil0 = subsample_spectra(rsoil0, R_down=1).to(
            device=device, dtype=N.dtype
        )
    ...
    [
        ...
        gammasdb,
        gammaso,
    ] = foursail(refl, trans, lidfa, lidfb, typelidf, lai, hspot, tts, tto, psi, rsoil0)
...
def run_sail(
    ...
    if rsoil0 is None:
        if (rsoil is None) or (psoil is None):
            raise ValueError(
                "If rsoil0 isn't define, then rsoil and psoil" " need to be defined!"
            )
        rsoil0 = rsoil * (psoil * soil_spectrum1 + (1.0 - psoil) * soil_spectrum2)
...
def run_thermal_sail(
    ...
    Lw = (
        rdot * Hsky
        + (aeev * Hc + gammasot * emv * (Hh - Hc) + aees * Hd + tso * ems * (Hs - Hd))
    ) / np.pi
