...
from .spectral_library import spectral_lib


def subsample_spectra(
...
def init_prospect_spectra(
    ...
    if prospect_version == "5":
        ...
        kbrown = (
            spectral_lib.prospectpro.kbrown.to(device) if kbrown is None else kbrown
        )
...
def run_prospect(
    ...
    """
    if init_spectra:
        [nr, kab, kcar, kbrown, kw, km, kant, kprot, kcbc, lambdas] = (
            init_prospect_spectra(
                nr=nr,
                kab=kab,
                kcar=kcar,
                kbrown=kbrown,
    ...
    tensors = [nr, kab, kcar, kbrown, kw, km, kant, kprot, kcbc, lambdas]
    tensors = [value.to(device=N.device, dtype=N.dtype) for value in tensors]
    nr, kab, kcar, kbrown, kw, km, kant, kprot, kcbc, lambdas = tensors
    alpha = torch.as_tensor(alpha, dtype=N.dtype, device=N.device)
    ant = torch.as_tensor(ant, dtype=N.dtype, device=N.device)
    prot = torch.as_tensor(prot, dtype=N.dtype, device=N.device)
    cbc = torch.as_tensor(cbc, dtype=N.dtype, device=N.device)
...
def calctav(alpha, nr):
    ...
    tp4 = (
        ...
        * n2**2
        * (n2**2 + 1)
        * torch.log((2 * npx * b - nm**2) / (2 * npx * a - nm**2))
        / (npx**3 * nm**2)
    )
    tp5 = (
        16
...
def refl_trans_one_layer(alpha, nr, tau):
    ...
    ralf = 1.0 - talf
    t12 = calctav(nr.new_tensor(90.0), nr)
    r12 = 1.0 - t12
    t21 = t12 / (nr * nr)
    r21 = 1 - t21
...
def prospect_d(
    ...
    if lambdas is None:
        if R_down > 1:
            lambdas = torch.arange(400, 2500, R_down, dtype=N.dtype, device=N.device)
        else:
            lambdas = torch.arange(400, 2501, dtype=N.dtype, device=N.device)
    n_lambdas = len(lambdas)
    # TODO:  Perform only at intialization
    n_elems_list = [
        len(spectrum) for spectrum in (nr, kab, kcar, kbrown, kw, km, kant, kprot, kcbc)
    ]
